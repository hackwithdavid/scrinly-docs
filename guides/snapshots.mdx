...
Snapshots are synchronous by default. Set `async=true` or provide `webhookUrl` to queue the render and receive a job ID. `spec=true` is always synchronous and rejects async or webhook combinations.
