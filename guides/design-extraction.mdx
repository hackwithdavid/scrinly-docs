...
Raw `data:` and `blob:` font URLs are dropped. HTTP(S) sources are length- and count-capped for the extraction response, while AI projections remove font URLs completely and retain only delivery kinds such as `self-hosted`, `google-fonts`, or `cdn`.
