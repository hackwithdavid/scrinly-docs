...
AI design-spec requests may include an ephemeral provider key in a separate header:

```http
X-Scrinly-Provider-API-Key: your_provider_key
```

`Authorization` still contains the Scrinly key. Provider keys are accepted only in this header and are not stored, echoed, or logged. See [AI design specifications](/guides/ai-design-specs).
